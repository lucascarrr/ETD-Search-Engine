...
(function (global, factory) {
  ...
  */
  ...
  var testLineShape = new shapes__namespace.Line();
  ...
  */
  function stackOrderAscending(series) {
    ...
    return d3Shape__namespace.stackOrderNone(series).sort(function (a, b) {
    ...
    function Plot() {
      ...
      _this4._shapeConfig = d3plusCommon.assign(_this4._shapeConfig, {
        Area: {
          ...
          labelBounds: function labelBounds(d, i, aes) {
            var r = shapes__namespace.largestRect(aes.points, {
              angle: d3Array.range(-20, 20, 5)
            });
            if (!r || r.height < 20 || r.width < 50) r = shapes__namespace.largestRect(aes.points, {
      ...
      _this4._timelineConfig = d3plusCommon.assign(_this4._timelineConfig, {
        ...
        brushMin: function brushMin() {
          return _this4._xTime || _this4._yTime || _this4._x2Time || _this4._y2Time ? 2 : 1;
        }
      });
    ...
    _createClass(Plot, [{
      ...
      */
      ...
      value: function _draw(callback) {
        ...
        if (this._stacked) {
          var _domains;
          var stackedData = data.filter(function (d) {
            return ["Area", "Bar"].includes(d.shape);
          });
          ...
          });else if (order === d3Shape__namespace.stackOrderNone) stackKeys.sort(function (a, b) {
            return a.localeCompare(b);
          });
          stackData = d3Shape__namespace.stack().keys(stackKeys).offset(this._stackOffset).order(order instanceof Array ? d3Shape__namespace.stackOrderNone : order).value(function (group, key) {
          ...
          domains = (_domains = {}, _defineProperty(_domains, this._discrete, this["_".concat(this._discrete, "Time")] ? d3Array.extent(discreteData) : discreteData), _defineProperty(_domains, opp, [d3Array.min(stackData.map(function (g) {
          ...
          domains = {
            x: !xTime && this._discrete === "x" || this._xSort ? xData : d3Array.extent(xData),
            x2: !x2Time && this._discrete === "x" || this._x2Sort ? x2Data : d3Array.extent(x2Data),
            y: !yTime && this._discrete === "y" || this._ySort ? yData : d3Array.extent(yData),
            y2: !y2Time && this._discrete === "y" || this._y2Sort ? y2Data : d3Array.extent(y2Data)
          };
        }
        ...
        var _x3 = scales__namespace["scale".concat(xScale)]().domain(domains.x).range(d3Array.range(0, width + 1, width / (domains.x.length - 1))),
        ...
        var x2Ticks = x2Scale === "Point" && x2Ticks instanceof Array && x2Ticks.every(function (t) {
          return barLabels.includes("".concat(t));
        }) ? [] : undefined;
        var xTicks = xScale === "Point" && xTicks instanceof Array && xTicks.every(function (t) {
          return barLabels.includes("".concat(t));
        }) ? [] : undefined;
        var y2Ticks = y2Scale === "Point" && y2Ticks instanceof Array && y2Ticks.every(function (t) {
          return barLabels.includes("".concat(t));
        }) ? [] : undefined;
        var yTicks = yScale === "Point" && yTicks instanceof Array && yTicks.every(function (t) {
          return barLabels.includes("".concat(t));
        }) ? [] : undefined;
        if (showY) {
        ...
        new shapes__namespace.Rect().data([{}]).select(rectGroup.node()).x(xRange[0] + (xRange[1] - xRange[0]) / 2).width(xRange[1] - xRange[0]).y(this._margin.top + topOffset + yRange[0] + (yRange[1] - yRange[0]) / 2).height(yRange[1] - yRange[0]).config(this._backgroundConfig).render();
        ...
        var exitAnnotations = this._previousAnnotations.filter(function (d) {
          return !annotationShapes.includes(d);
        });
        exitAnnotations.forEach(function (shape) {
          new shapes__namespace[shape]().data([]).select(annotationGroup).render();
        });
        ...
        var shapeConfig = {
          ...
          x: function x(d) {
            return d.x2 !== undefined ? _x3(d.x2, "x2") : _x3(d.x);
          },
          x0: discrete === "x" ? function (d) {
            return d.x2 ? _x3(d.x2, "x2") : _x3(d.x);
          } : _x3(typeof this._baseline === "number" ? this._baseline : domains.x[0]),
        ...
        shapeData.forEach(function (d) {
          ...
          if (_this6._stacked && ["Area", "Bar"].includes(d.key)) {
            var scale = opp === "x" ? _x3 : _y2;
          ...
          var s = new shapes__namespace[d.key]().config(shapeConfigInner).data(d.values);
          if (d.key === "Bar") {
            var space;
            var _scale = _this6._discrete === "x" ? _x3 : _y2;
            ...
            if (scaleType !== "Point" && vals.length === 2) {
              var allPositions = Array.from(new Set(d.values.map(function (d) {
                return _scale(d[_this6._discrete]);
              })));
            ...
            if (d3Array.max(groups.map(function (d) {
              ...
              var xMod = scales__namespace.scaleLinear().domain([0, uniqueIds.length - 1]).range([-offset, offset]);
              s[_this6._discrete](function (d, i) {
            ...
            if (_this6._confidence) {
              ...
              var scaleFunction = _discrete2 === "x" ? _y2 : _x3;
              ...
              var area = new shapes__namespace.Area().config(areaConfig).data(d.values);
          ...
          var _loop2 = function _loop2(e) {
            s.on(shapeEvents[e], function (d, i, x, event) {
              return _this6._on[shapeEvents[e]](d.data, d.i, x, event);
            });
          };
          ...
          var _loop3 = function _loop3(e) {
            s.on(classEvents[e], function (d, i, x, event) {
              return _this6._on[classEvents[e]](d.data, d.i, x, event);
            });
          };
          ...
          if (d.key === "Line") {
            var markers = new shapes__namespace.Circle().data(_this6._lineMarkers ? d.values : []).config(shapeConfig).config(_this6._lineMarkerConfig).id(function (d) {
              return "".concat(d.id, "_").concat(d.discrete);
            });
            var _loop4 = function _loop4(e) {
              markers.on(globalEvents[e], function (d, i, x, event) {
                return _this6._on[globalEvents[e]](d.data, d.i, x, event);
              });
            };
            ...
            var _loop5 = function _loop5(e) {
              markers.on(shapeEvents[e], function (d, i, x, event) {
                return _this6._on[shapeEvents[e]](d.data, d.i, x, event);
              });
            };
            ...
            var _loop6 = function _loop6(e) {
              markers.on(classEvents[e], function (d, i, x, event) {
                return _this6._on[classEvents[e]](d.data, d.i, x, event);
              });
            };
  ...
  */
  var BoxWhisker = /*#__PURE__*/function (_Plot3) {
    ...
    */
    ...
    return _createClass(BoxWhisker);
  }(Plot);
  /**
      @class BumpChart
      @extends Plot
  ...
  */
  var StackedArea = /*#__PURE__*/function (_AreaPlot) {
    ...
    */
    ...
    return _createClass(StackedArea);
  }(AreaPlot);
