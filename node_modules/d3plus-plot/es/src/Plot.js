    ...
    */
    ...
    value: function _draw(callback) {
      ...
      if (this._stacked) {
        ...
        domains = (_domains = {}, _defineProperty(_domains, this._discrete, this["_".concat(this._discrete, "Time")] ? extent(discreteData) : discreteData), _defineProperty(_domains, opp, [min(stackData.map(function (g) {
      ...
      var _x3 = scales["scale".concat(xScale)]().domain(domains.x).range(range(0, width + 1, width / (domains.x.length - 1))),
      ...
      exitAnnotations.forEach(function (shape) {
        new shapes[shape]().data([]).select(annotationGroup).render();
      });
      ...
      shapeData.forEach(function (d) {
        ...
        if (_this3._stacked && ["Area", "Bar"].includes(d.key)) {
        ...
        if (d.key === "Bar") {
          var space;
          var _scale = _this3._discrete === "x" ? _x3 : _y2;
          ...
          if (scaleType !== "Point" && vals.length === 2) {
            var allPositions = Array.from(new Set(d.values.map(function (d) {
              return _scale(d[_this3._discrete]);
            })));
        ...
        var _loop2 = function _loop2(e) {
          s.on(shapeEvents[e], function (d, i, x, event) {
            return _this3._on[shapeEvents[e]](d.data, d.i, x, event);
          });
        };
        ...
        var _loop3 = function _loop3(e) {
          s.on(classEvents[e], function (d, i, x, event) {
            return _this3._on[classEvents[e]](d.data, d.i, x, event);
          });
        };
        ...
        if (d.key === "Line") {
          var markers = new shapes.Circle().data(_this3._lineMarkers ? d.values : []).config(shapeConfig).config(_this3._lineMarkerConfig).id(function (d) {
